<template>
    <div class="text-field tool" attr="star">
      ...
      >
        {{ this.$store.getters.getAddToPageLastName }}
      </p>
  ...
  export default {
    ...
    computed: {
      hasTextvalue() {
        return this.$store.getters.getAddToPageLastName != undefined
      },
      isSign() {
    ...
    methods: {
      ...
      confirmStarAction() {
        if (
          ...
          return
        !this.hasTextvalue && this.$BUS.$emit('addTextToPage', 'last_name')
