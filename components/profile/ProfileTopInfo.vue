<template>
  <section class="flex flex-wrap w-full md:justify-between font-family">
    ...
    <div class="bg-white md:w-3/12 lg:w-[19%] w-full profile-image-container">
      <div class="icon-img relative" @click="visibleUploadImageDialog = true">
      ...
      <div v-if="isUser" class="text-wrapper" @click="visibleUploadImageDialog = true">Click to upload an image</div>
    </div>
...
export default mixins(login).extend({
  ...
  // },
  components: { CropperImageUpload, Pencil, SpinnerDottedIcon, Cropper, VerticalButtons, SquareButton, ballloader },
  methods: {
    getTeamPublicFolder() {
      this.$axios.get()
    },
