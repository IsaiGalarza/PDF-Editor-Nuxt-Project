<template>
  <div class="mt-[70px] lg:mt-0 px-2">
      ...
      >
        <div class="w-full p-4 pb-0 sm:hidden">
          <div class="flex items-center bg-gray-100 rounded text-sm w-full">
            <div
              class="flex items-center justify-center h-10 w-1/2 cursor-pointer"
              :class="{ 'text-white bg-gray-500 rounded': !showFolders }"
    ...
    <div
      v-if="isFetched"
      class="bg-white rounded-xl pb-8 px-2"
      :class="{ 'hidden sm:block': showFolders }"
    >
      <div
        class="rounded-2xl min-w-[300px] overflow-x-auto custom-scrollbar relative"
      >
            ...
            <tr v-for="(item, i) in files" :key="i">
              ...
              <td class="row">
                <div class="flex items-center">
                  ...
                  <p
                    class="text-[#414142] whitespace-nowrap truncate text-[15px]"
                  >
                    <span
                      @click="
                        routeToFileManager(
              ...
              <td class="hidden lg:table-cell">
                <p
                  v-show="(item || {}).fileAction != 'doNotPost'"
                  class="capitalize"
                >
                  {{ (item || {}).fileAction || '' }}
                </p>
              ...
              <td class=" ">
                <div
                  @click="shareLinkFunc(item.paperLink, item.fileName)"
...
export default Vue.extend({
  ...
  methods: {
    ...
    routeToFileManager(val, privacy) {
      if (privacy == FilePrivacy.PRIVATE) this.showPrivateModal = true
        ...
        localStorage.setItem(
          'from_publicpage',
          JSON.stringify({ fromBusiness: true })
        )
    ...
    async getUserFiles(page, search) {
      await this.$axios
        .$get(
          `/files/?userId=${this.userInfo?.id}&fileName[$like]=${search}%&$skip=${page}&$sort[position]=1&filePrivacy[$ne]=doNotPost`,
          {
            params: {
              isEditing: 0,
            },
